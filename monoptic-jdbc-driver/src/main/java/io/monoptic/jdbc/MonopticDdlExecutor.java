...
/** Mostly copy-pasted from ServerDdlExecutor. */
public class MonopticDdlExecutor extends DdlExecutorImpl {
  /** Singleton instance. */
  public static final MonopticDdlExecutor INSTANCE = new MonopticDdlExecutor();
  ...
  public static final SqlParserImplFactory PARSER_FACTORY =
      new SqlParserImplFactory() {
        ...
        @Override public DdlExecutor getDdlExecutor() {
          return MonopticDdlExecutor.INSTANCE;
        }
      };
  ...
  static <T extends KubernetesTable> T kubernetesTable(CalcitePrepare.Context context, String table) {
    return (T) context.getRootSchema().getSubSchema("KUBERNETES", true).getTable(table, true).getTable();
  }
  ...
  static void erase(SqlIdentifier name, CalcitePrepare.Context context) {
    ...
    if (table instanceof MutableTable) {
      ...
      throw new UnsupportedOperationException("Only MutableTable supports truncate");
    }
  }

  //TODO [monoptic] populate remotely via engine
  ...
  public void execute(SqlCreateMaterializedView create,
    ...
    final MaterializedView materialized = new MaterializedView(x);

    SchemaPlus schemaPlus = pair.left.plus();
    try { 
      // Table does not exist. Create it.
      addView(context, pair.right, sql, true);
      pair.left.add(pair.right, materialized);
      //populate(create.name, create.query, context);
//      table.key =
//          MaterializationService.instance().defineMaterialization(pair.left, null,
//              sql, schemaPath, pair.right, MaterializedViewFactory.INSTANCE, true, true);
    } catch (Exception e) {
      throw new RuntimeException("Cannot CREATE MATERIALIZED VIEW in database '" + schemaPlus.getName() + "'.", e);
    }
  }
  ...
  public void execute(SqlCreateView create,
    ...
    Util.discard(x);
    try {
      addView(context, pair.right, sql, false);
    } catch (Exception e) {
      throw new RuntimeException("Cannot CREATE VIEW in namespace '" + schemaPlus.getName() + "'.", e);
    }
    schemaPlus.add(pair.right, viewTableMacro);
  }

  private void addView(CalcitePrepare.Context context, String name, String sql, boolean materialize) {
    KubernetesViewTable viewTable = kubernetesTable(context, "VIEWS");
    viewTable.create(name, sql, materialize);
  }
